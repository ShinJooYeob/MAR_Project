#include "stdafx.h"
#include "..\public\BreakedGazebo.h"
#include "Terrain.h"
#include "Player.h"



CBreakedGazebo::CBreakedGazebo(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CMonsterWeapon(pDevice,pDeviceContext)
{
}

CBreakedGazebo::CBreakedGazebo(const CBreakedGazebo & rhs)
	: CMonsterWeapon(rhs)
{
}

HRESULT CBreakedGazebo::Initialize_Prototype(void * pArg)
{
	FAILED_CHECK(__super::Initialize_Prototype(pArg));
	return S_OK;
}

HRESULT CBreakedGazebo::Initialize_Clone(void * pArg)
{
	...
	Add_JumpPower(pUtilMgr->RandomFloat(20, 25.f));
	m_fTurningTime = 0;
	//m_vRotAxis = m_vSpoutDir;
	return S_OK;
}

_int CBreakedGazebo::Update(_double fDeltaTime)
{
...
_int CBreakedGazebo::LateUpdate(_double fDeltaTime)
{
...
_int CBreakedGazebo::LightRender()
{
...
void CBreakedGazebo::CollisionTriger(_uint iMyColliderIndex, CGameObject * pConflictedObj, CCollider * pConflictedCollider, _uint iConflictedObjColliderIndex, CollisionTypeID eConflictedObjCollisionType)
{
...
HRESULT CBreakedGazebo::SetUp_Components()
{
...
void CBreakedGazebo::Add_JumpPower(_float power)
{
...
CBreakedGazebo * CBreakedGazebo::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, void * pArg)
{
	CBreakedGazebo*	pInstance = new CBreakedGazebo(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype(pArg)))
	{
		MSGBOX("Failed to Created CBreakedGazebo");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CBreakedGazebo::Clone(void * pArg)
{
	CBreakedGazebo*	pInstance = new CBreakedGazebo(*this);
