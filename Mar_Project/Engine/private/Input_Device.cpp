...
HRESULT CInput_Device::Ready_Input_Device(HINSTANCE hInst, HWND hWnd, _float fDoubleTimeInterver)
{
	...
	FAILED_CHECK(m_pInput->CreateDevice(GUID_SysKeyboard, &m_pKeyboard, nullptr));

	FAILED_CHECK(m_pKeyboard->SetDataFormat(&c_dfDIKeyboard));

	FAILED_CHECK(m_pKeyboard->SetCooperativeLevel(hWnd, DISCL_BACKGROUND | DISCL_NONEXCLUSIVE));

	FAILED_CHECK(m_pKeyboard->Acquire());

	FAILED_CHECK(m_pInput->CreateDevice(GUID_SysMouse, &m_pMouse, nullptr));

	FAILED_CHECK(m_pMouse->SetDataFormat(&c_dfDIMouse));

	FAILED_CHECK(m_pMouse->SetCooperativeLevel(hWnd, DISCL_BACKGROUND | DISCL_NONEXCLUSIVE));

	FAILED_CHECK(m_pMouse->Acquire());
